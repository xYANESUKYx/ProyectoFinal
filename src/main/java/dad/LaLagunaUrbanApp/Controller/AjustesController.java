...
public class AjustesController implements Initializable {
	...
	private EventHandler<MouseEvent> onChangePass;
	...
	public void initialize(URL location, ResourceBundle resources) {
		nombreTextField.textProperty().addListener((observable, oldValue, newValue) -> {
	        if (!newValue.isEmpty()) {guardarButton.setVisible(true);
	        } else {guardarButton.setVisible(false);}
	    });
		edadTextField.textProperty().addListener((observable, oldValue, newValue) -> {
	        if (!newValue.isEmpty()) {guardarButton.setVisible(true);
	        } else {guardarButton.setVisible(false);}
	    });
		pesoTextField.textProperty().addListener((observable, oldValue, newValue) -> {
	        if (!newValue.isEmpty()) {guardarButton.setVisible(true);
	        } else {guardarButton.setVisible(false);}
	    });
		alturaText.textProperty().addListener((observable, oldValue, newValue) -> {
	        if (!newValue.isEmpty()) {guardarButton.setVisible(true);
	        } else {guardarButton.setVisible(false);}
	    });
		...
		pesoTextField.textProperty().addListener((o, ov, nv) -> {
			...
			pesoObjetivoLabel.setText(pesoObjetivo()+ "kg");
		});
	}
    ...
    void onPassClicked(MouseEvent event) {
    	if(onChangePass != null) onChangePass.handle(event);
    }
    
    public void setonChangeUser(EventHandler<MouseEvent> onChangePass) {
	...
	String pesoObjetivo() {
		...
		if(estado.equals(objetivo)) {
			...
			double dif =  Integer.parseInt(objetivo) - Integer.parseInt(estado);
			dif = (dif * 2) + peso;
			return "" + dif;
		}
	}
    ...
    void onInicioClicked(MouseEvent event) {
	 ...
	 void onGuardarClicked(MouseEvent event) {
		 ...
		 if (nombreTextField != null && !nombreTextField.getText().isEmpty()) {
			...
			nombreLabel.setText(nombreTextField.getText());
		}
		...
		try {actualizarUsuario();} catch (SQLException e) {System.out.println("Error" + e.getMessage());}

    	if(onGuardar != null) onGuardar.handle(event);
	  	}
